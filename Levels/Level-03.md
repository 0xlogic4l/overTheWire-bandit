 # Bandit Level 3 → Level 04

> **Date:** 2026-06-29

> **Difficulty:** ☆☆☆☆☆

> **Status:** ✅ Completed

---

# Level Goal

The password for the next level is stored in a hidden file in the `inhere` directory.

---

# Enumeration

```bash
# Commands used for reconnaissance

bandit3@bandit:~$ ls
bandit3@bandit:~$ cd inhere
bandit3@bandit:~$ ls -a
```

### Output

```text
inhere

bandit3@bandit:~/inhere$

.  ..  ...Hiding-From-You
```

---

# Analysis

* This level serves as an introduction to the Bandit wargame.
* The objective is to connect to the Bandit1 server using SSH.
* The provided username and password are used for authentication.
* After logging in, the password for the next level can be found in the `...Hiding-From-You` file located in the home directory.

---

# Solution

```bash
# Final commands used to solve the level
bandit3@bandit:~$ cat ...Hiding-From-You 
```

### Output

```text
xzTXq1rDJQVVAzdv5cHq1TQytTWufAMq
```

---

# 🚩 Flag

```text
xzTXq1rDJQVVAzdv5cHq1TQytTWufAMq
```

---

# Explanation

This challenge about files Hidden files and learn how i read it `...Hiding-From-You`

---

# Commands Learned

| Command  | Purpose               |
| -------- | --------------------- |
| `cd`     | Change Directory       |
| `ls -a`  | Listing files         |
| `cat`    | Display file contents |
| `ssh`    | Connect to connect remotly server    |

---

# Notes

> 💡 Add anything worth remembering for future levels.

* SSH is used to securely connect to a remote Linux server.
* The default SSH port is **22**, but Bandit uses port **2220**.
* The `cat` command displays the contents of a file.
* Always read the challenge description carefully, as it usually contains important connection details.
* The password obtained in each level is used to log in to the next level.


---

# Common Mistakes

* Using the wrong username or password when connecting via SSH.
* Forgetting to specify the correct port (`-p 2220`).
* Attempting to use command before logging in to the Bandit server.
* Typing the SSH command with incorrect syntax.
* Looking for the password in the wrong directory instead of the home directory.

---

# Takeaways

* Learned how to listing hidde files using `ls -a` command.
* Learned how i change current directory to another using `cd` command
* Practiced reading the contents of a files named with special charachter using the `cat` command.

---

# References

* https://overthewire.org/wargames/bandit/
* `man <command>`
* Linux documentation
